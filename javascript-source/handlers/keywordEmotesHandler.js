...
(function () {
    ...
    $.bind('ircChannelMessage', function (event) {
        ...
        keywords.forEach((json, keyword) => {
            let count = 0;
            if (json.isRegex) {
                let re = new RegExp(keyword, 'g' + (json.isCaseSensitive ? 'i' : ''));
                count = (message.match(re) || []).length;
            } else {
                // work in pure lowercase if the keyword is case insensitive
                let haystack = json.isCaseSensitive ? message : message.toLowerCase();
                let needle = json.isCaseSensitive ? keyword : keyword.toLowerCase();

                let lastPosition = haystack.indexOf(needle);
